...
export default function StoreLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
      <>
        <StoreHeader onToggleSearch={handleSearchClick} onToggleCart={handleCartClick} />
        <div className="min-h-screen bg-white text-white">{children}</div>
        <SlideOverPanel open={Boolean(openSearchPanel)} onClose={() => { setOpenSearchPanel(null); }} title="search" />
        <SlideOverPanel open={Boolean(openCartPanel)} onClose={() => { setOpenCartPanel(null); }} title="cart" />
      </>
  );
}
